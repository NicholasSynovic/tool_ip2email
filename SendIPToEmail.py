...
class Email:
	...
	# Some of the code below came from: https://realpython.com/python-send-email
	...
	def send(self, message:str)	->	tuple:
		...
		with smtplib.SMTP_SSL(host='smtp.gmail.com', port=port, context=context) as gmail: 
			try:
				gmail.login(user=sender[0], password=sender[1])
			except smtplib.SMTPAuthenticationError:
				print('You need to turn on "Less secure app access" in your Google account before you can use this program (https://myaccount.google.com/lesssecureapps).\nIf this is not the issue then your EMAIL or PASSWORD is probably incorrect.')
				exit(1)
...
def program()	->	None:
	...
	email = Email(senderEmail=senderEmail, senderEmailPassword=senderPassword, recieverEmail=reciverEmail)
